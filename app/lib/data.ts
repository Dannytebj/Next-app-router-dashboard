...
import { client } from './db-connections';
import {
  CustomerField,
  CustomersTableType,
  InvoiceForm,
...
export async function fetchInvoicesPages(query: string) {
  try {
    ...
    WHERE
      ...
      invoices.status ILIKE $1
  `;
    const count = await client.query(queryText,[`%${query}%`]);
...
export async function fetchInvoiceById(id: string) {
  try {
    const text = `
      ...
      WHERE invoices.id = $1;
    `;
    const data: QueryResult<InvoiceForm> = await client.query(text,[id]);
...
export async function fetchFilteredCustomers(query: string) {
  try {
    const data: QueryResult<CustomersTableType> = await client.query(`
		SELECT
		  customers.id,
		  customers.name,
		  customers.email,
